...
void main()
{
   ...
   if (delta > 0.005 || normal.y == 0.0) {
      vec3 final = clamp(light_color(water_color, normal, fragPos, viewPos, lightPos), 0.0, 1.0);
      ...
      FragColor = clamp(vec4(light_color(color.xyz, normal, fragPos, viewPos, lightPos), color.a), 0.0, 1.0);
   }
}